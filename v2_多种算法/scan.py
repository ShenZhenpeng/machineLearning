...
if __name__ == "__main__":
   ...
   for data, filename in locator.search_file_path(args, valid_regex):
       """
       noComments = subprocess.Popen("php -w " + filename, shell=True, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.STDOUT).stdout.read()
       dstr = r'"(?:[^"\\]|\\.)*"'
       sstr = r"'(?:[^'\\]|\\.)*'"
       _PHP_TAG = re.compile(r'''<\?[^"']*?(?:(?:%s|%s)[^"']*?)*(?:\?>|$)''' % (dstr, sstr))
       phpContent=re.findall(_PHP_TAG, noComments)
       data = ""
       for phpBlock in phpContent:
           data += phpBlock
       """
       if data:
